...
document.addEventListener("DOMContentLoaded", () => {
    // Initialize todayFormatted safely after all functions are fully parsed
    todayFormatted = formatDateString(todayDate);
...
function savePostsToLocalStorage() {
    // Demonstration mode: in-memory only, no persistence to localStorage
}
...
function removeSelectedImage(event) {
    // Commented out upload feature stub to prevent ReferenceErrors
...
// Safe wrapper for Lucide icon creation in offline/isolated environments
function safeCreateIcons() {
    try {
        if (typeof lucide !== 'undefined' && lucide.createIcons) {
            lucide.createIcons();
        } else {
            console.warn("Lucide icons library is not loaded. Icons will be skipped.");
        }
    } catch (e) {
